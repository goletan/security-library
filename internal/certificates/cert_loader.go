...
func (cl *CertLoader) checkFilePermissions(filePath string) error {
	...
	if err != nil {
		cl.obs.Logger.Error("unable to access file: %w", zap.Error(err))
	...
	if info.Mode().Perm()&(syscall.S_IRWXG|syscall.S_IRWXO) != 0 {
		cl.obs.Logger.Error("file %s has too permissive permissions: %w", zap.String("filePath", filePath), zap.Error(err))
	...
	if info.Mode().Perm() != 0600 && info.Mode().Perm() != 0400 {
		cl.obs.Logger.Error(fmt.Sprintf("file %s has invalid permissions: %o, expected 600 or 400: %w", filePath, info.Mode().Perm()), zap.Error(err))
